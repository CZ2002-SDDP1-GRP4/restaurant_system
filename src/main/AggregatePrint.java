...
/**
 * An abstract class defining an object that does three things: 1) Processes an
 * ungrouped array of objects including MenuItems 2) Aggregates MenuItems with
 * Quantity. 3) Prints those MenuItems with Quantity. This abstract class will
 * be used by the OrderInvoiceApp and SalesReportApp to process, aggregate and
 * print. They share common attributes such as an array of MenuItems to print
 * and an array indicating quantity, and a common method to aggregate MenuItems
 * with Quantity, but their processing to extract the relevant MenuItems and
 * their printing are different (e.g. OrderInvoice needs to show table number
 * and time and discounts etc, but SalesReport only needs the revenue)
 * 
 * @author Bryan
 * @version 1.0
 */
